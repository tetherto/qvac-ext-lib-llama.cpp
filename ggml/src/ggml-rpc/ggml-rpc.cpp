...
static void ggml_backend_rpc_buffer_get_tensor_2d(ggml_backend_buffer_t buffer, const ggml_tensor * tensor, void * data,
    ...
    if (stride_data == size) {
        bool status =
            ctx->cmd_queue->submit_rpc_sync(RPC_CMD_GET_TENSOR_2D, &request, sizeof(request), data, size * n_copies);
        RPC_STATUS_ASSERT(status);
    } else {
...
static void add_tensor(ggml_tensor *                              tensor,
    ...
    for (int i = 0; i < GGML_MAX_SRC; i++) {
        add_tensor(tensor->src[i], cmd_queue, tensors, visited);
    }
    add_tensor(tensor->view_src, cmd_queue, tensors, visited);
    tensors.push_back(serialize_tensor(tensor, cmd_queue));
}
...
static void serialize_graph(uint32_t                                   device,
    ...
    for (uint32_t i = 0; i < n_nodes; i++) {
        add_tensor(cgraph->nodes[i], cmd_queue, tensors, visited);
    }
...
bool rpc_server::comm_init(const rpc_msg_comm_init_req & request, rpc_msg_comm_init_rsp & response) {
    ...
    if (request.rank == 0) {
        ...
        state.peer = srv->accept(5000);
        if (state.peer == nullptr) {
            return true;
        }
        if (!state.peer->set_timeout(5000) || !state.peer->recv_data(remote_caps, sizeof(remote_caps))) {
            state.peer = nullptr;
            return true;
        }
        state.peer->get_caps(local_caps);
        if (!state.peer->send_data(local_caps, sizeof(local_caps)) || !state.peer->set_timeout(-1)) {
            state.peer = nullptr;
            return true;
        }
        ...
        if (!state.peer->set_timeout(5000)) {
            state.peer = nullptr;
            return true;
        }
        state.peer->get_caps(local_caps);
        if (!state.peer->send_data(local_caps, sizeof(local_caps)) ||
            !state.peer->recv_data(remote_caps, sizeof(remote_caps)) || !state.peer->set_timeout(-1)) {
            state.peer = nullptr;
            return true;
        }
        state.peer->update_caps(remote_caps);
...
static void * ggml_backend_rpc_comm_init(ggml_backend_t * backends, size_t n_backends) {
    ...
    bool ok = completion_ok(completions[1]);
    if (!ok) {
        GGML_LOG_WARN("%s: rank 1 (%s) failed to initialize\n", __func__, ranks[1].endpoint.c_str());
        completion_ok(completions[0]);
        return nullptr;
    }
    if (!completion_ok(completions[0])) {
        GGML_LOG_WARN("%s: rank 0 (%s) failed to initialize\n", __func__, ranks[0].endpoint.c_str());
        ok = false;
    }
    if (!ok) {
        for (const auto & rank : ranks) {
