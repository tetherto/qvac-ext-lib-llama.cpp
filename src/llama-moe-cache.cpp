...
struct layer_source {
    int32_t layer;
    std::vector<std::pair<projection_kind, ggml_tensor *>> projections;
};

static std::vector<std::pair<projection_kind, ggml_tensor *>> get_layer_projections(const llama_layer & layer) {
    std::vector<std::pair<projection_kind, ggml_tensor *>> result;
...
struct llama_moe_cache::impl {
    ...
    impl(
        ...
        std::vector<std::pair<projection_kind, ggml_tensor *>> reference;
        ...
        for (size_t il = 0; il < model.layers.size(); ++il) {
            ...
            if (!all_host) {
                throw std::runtime_error("MoE cache requires complete host-resident expert banks");
            }

            if (reference.empty()) {
                reference = layer_projections;
            } else {
                if (reference.size() != layer_projections.size()) {
                    throw std::runtime_error("MoE cache requires uniform expert projections");
                }
                for (size_t ip = 0; ip < reference.size(); ++ip) {
                    if (reference[ip].first != layer_projections[ip].first ||
                        !same_layout(reference[ip].second, layer_projections[ip].second)) {
                        throw std::runtime_error("MoE cache requires uniform expert tensor layouts");
                    }
                }
            }
            source_layers.push_back({ int32_t(il), std::move(layer_projections) });
        }

        if (source_layers.empty()) {
            throw std::runtime_error("MoE cache found no eligible host-resident expert banks");
        }
    ...
    bool prepare(
        ...
        for (const llama_moe_cache_lru_fill & fill : fills) {
            for (size_t binding_id : cache_layer.bindings) {
                ...
                const size_t src_available = ggml_nbytes(source_binding.source) - src_offset;
                const size_t copy_size = std::min(cache_projection.expert_size, src_available);
                if (source_binding.source->data == nullptr || copy_size == 0) {
                    return false;
                }
