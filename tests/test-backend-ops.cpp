...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    // FLASH_ATTN_EXT tests are skipped because they cause a GPU shader hang on NVIDIA T4 (Turing)
    // with logit_softcap != 0 + prec=def. Re-enable with GGML_TEST_FLASH_ATTN_EXT=1.
    if (getenv("GGML_TEST_FLASH_ATTN_EXT")) {
